...
:: =================================================================
:: SKYRIM CONTROL CENTER ENGINE INITIALIZATION (.INI FILE DATABASE)
:: =================================================================
...
echo   [2] UTILITY: Launch Mod Manager (Background)
...
:HUB_VORTEX
cls
echo Starting external background process for Vortex...
if exist "%VORTEX_EXE%" (
    start "" "%VORTEX_EXE%"
) else (
    echo [WARNING] Vortex executable not found at specified path.
    pause
)
goto MAIN_MENU
...
cls
echo =================================================================
echo                   [3] DIRECTORY & LOG DISCOVERY HUB
echo =================================================================
echo   [1] Open Root Skyrim Common Installation Folder
echo   [2] Open Vortex Mod Staging Folder
echo   [3] Open SKSE Crash Log Dump Directory (Windows Explorer)
echo   [4] 🔍 SCAN LATEST CRASH LOG (Analyze directly in CMD)
...
cls
echo =================================================================
echo             SKSE LAUNCHER LOG ^& CRASH ANALYZER
echo =================================================================
set "LOG_DIR=%DOCS_BASE%\My Games\Skyrim Special Edition\SKSE"

if not exist "%LOG_DIR%" (
    echo  [ERROR] Could not find the SKSE directory at:
    echo  "%LOG_DIR%"
    echo  Please verify your Documents path in the setup menu.
    echo.
    pause
    goto SUB_MENU_EXPLORER
)

:: Step into target directory and isolate the single newest log file
pushd "%LOG_DIR%"
set "LATEST_LOG="
for /f "delims=" %%F in ('dir /b /o-d *.log *.txt 2^>nul') do (
    if not defined LATEST_LOG set "LATEST_LOG=%%F"
)
popd

if "%LATEST_LOG%"=="" (
    echo  [NOTICE] No crash log or text dump files found in this folder.
    echo  Your game configuration workspace looks clean!
    echo.
    pause
    goto SUB_MENU_EXPLORER
)

echo  Found target log: %LATEST_LOG%
echo  Analyzing crash signatures... please wait...
echo -----------------------------------------------------------------
echo.

echo =================== CRASH SIGNATURE ANALYSIS ===================
:: Fast pipeline stream bypasses the Windows access denial block entirely
set "found_lines=0"
for /f "delims=" %%L in ('findstr /i "PROBABLE_CALL_STACK EXCEPTION_ACCESS_VIOLATION .esp .esm .esl .swf .nif BGS" "%LOG_DIR%\%LATEST_LOG%" 2^>nul') do (
    echo   -^> %%L
    set /a found_lines+=1
)

if %found_lines%==0 (
    echo   [!] No obvious crash markers found in the first layer scan.
    echo       Showing top 15 lines of the file instead:
    echo.
    powershell -NoProfile -Command "Get-Content '%LOG_DIR%\%LATEST_LOG%' -TotalCount 15"
)
echo =================================================================
echo.
echo  👉 The full file is safely openable via Option [3] if needed.
echo.
pause
goto SUB_MENU_EXPLORER
...
cls
echo =================================================================
echo                     [4] GAME LAUNCH PIPELINE
echo =================================================================
echo   [1] 🚀 LAUNCH VIA SKSE LOADER (Recommended for Mods)
echo   [2] 🔒 LAUNCH VIA VANILLA GAME EXE (Stock Engine)
echo   [B] Back to Main Menu
echo =================================================================
echo.
set "launch_choice="
...
echo  [2] Auditor: Creation Club ^& Master Checks
echo  [3] System: Dirty Plugins ^& Save File Health
echo  [4] Scan: Recent Plugin Modifications (Last 30 Days)
echo  [5] Tool: Archive Old Save Games (Keep 50)
echo  [6] AUDITOR: MISSING MASTER DEPENDENCIES
echo  [X] Exit to Main
echo ==========================================================
...
cls
echo ==========================================================
echo               AUDITOR: CREATION CLUB CHECK
echo ==========================================================
powershell -NoProfile -Command ^
...
cls
echo ==========================================================
echo           AUDITOR: MISSING MASTER DEPENDENCIES
echo ==========================================================
:: We use -Encoding Byte to read the file header safely without -Raw
powershell -NoProfile -Command ^
    "$plugins = Get-ChildItem '%DATA_DIR%' -Filter *.esp;" ^
    "foreach ($p in $plugins) {" ^
    "    $content = Get-Content $p.FullName -Encoding Byte -TotalCount 200;" ^
    "    $str = [System.Text.Encoding]::ASCII.GetString($content);" ^
    "    if ($str -match '(?i)Error: Missing Master') {" ^
    "        Write-Host ' [!] MISSING MASTER IN:' $p.Name -ForegroundColor Red;" ^
    "    } else {" ^
...
cls
echo =================================================================
echo             SKYRIM CONTROL CENTER INITIAL SETUP WIZARD
echo =================================================================
echo   [1] Standard CMD Mode (Copy/Paste paths Mode)
echo   [2] Interactive PowerShell (File Explorer Mode)
echo =================================================================
set "mode_choice="
...
if "%MODE%"=="CMD" (
    ...
    set /p "VORTEX_EXE=Paste full path to main .exe: "
) else (
...
echo.
echo   💡 Know your path already? Skip the scout and lock it in directly:
